...
class MyPost extends Component {
    constructor(props){
        ...
        this.state = {
            ...
            cantidadDeLikes: this.props.data.item.data.likes?.length, //length del array de likes.
            cantidadDeComentarios: this.props.data.item.data.comentarios
        }
    }
    componentDidMount(){
        //chequear si esta logueado preguntando en el if si "myLike" incluye al usuario logueado
        let myLike = this.props.data.item.data.likes?.includes(auth.currentUser.email)
        ...
        return( //primero <Image>
            ...
            <View style={styles.posteo}>
                ...
                <View style={styles.data}>
                    ...
                    <Text style={{fontSize:16, paddingBottom:8}} onPress={()=> this.props.navigation.navigate('ProfileUser',  { email: this.props.data.item.data.owner })} >
                        {this.props.data.item.data.owner}
                    </Text>
